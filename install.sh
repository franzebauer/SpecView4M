...
fetch     1vNte8khEMBbNVoQgnCP7vI-EhdfAIpr3 spectra_headers.csv               # REQUIRED index

fetch_tar 1-_wKDx3OMNMEbDYG2q7hh3V8-l9DIAQV SPV_LR_L1ver.tar.gz               # -> LR4/  (L1 combo, blue default)
fetch_tar 18Gu0iPxqpbVhooeq-Rq7K168NyN8GwGn SPV_HIZ.tar.gz                    # -> HIZ4/ (HIZ L1 combo)
...
if [ "$FULL" -eq 1 ]; then
  echo "==> Optional data (LR overlay, subsurvey catalogue, redshifts, conditions)"
  ...
  fetch     1IEfL9-MtN61dA0BTYccCZGIklRrppYX7 IWG3_XGAL_SPV_20260201T2230Z_target_catalogue.fits
  fetch     1tpWpyeC5Hv7lvOlfVqyFG-0n0USe25q5 conditions.csv                  # seeing cut for plot_desi_ratio.py

  # --- unclear datasets — uncomment once their role is confirmed ---
  # fetch_tar 1Jqv44crctWA8sm7RVOUavikhz8_4YiBW S1601.tar.gz                  # ??? contents
  # fetch     1HmYz19x-9KuF7nNaPxx1ktPs0JnQMpIh SPV_objects.fits              # ??? role in the viewer
fi

echo
echo "==> Done."
echo "Run:  python viewer.py spectra_headers.csv"
